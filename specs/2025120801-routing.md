...
│  │  - React (.tsx) or Vue (.vue) components               │ │
│  │  - Shares top-level package.json                       │ │
│  │  - API client wrapper                                   │ │
│  └────────────────────────┬───────────────────────────────┘ │
└───────────────────────────┼──────────────────────────────────┘
                            │ HTTP/JSON
...
/
├── lib/                          # 📦 LIBRARY CODE
│   ├── router/                   # Core routing primitives
│   │   ├── index.ts             # Router class & route registration
│   │   ├── types.ts             # Route, Handler, Middleware types
│   │   └── response.ts          # Response helpers (json, error, etc)
│   └── adapters/                # Runtime adapters
│       └── bun.ts               # Bun HTTP server adapter
│       # cloudflare.ts          # Cloudflare Workers adapter (future)
│       # node.ts                # Node.js adapter (future)
│
├── src/                          # 🏠 USER IMPLEMENTATION
│   ├── routes.ts                # HTTP route definitions (Laravel-style)
│   ├── server.ts                # HTTP server entry point
│   # cli.ts                     # CLI entry point (Phase 4)
│   # commands.ts                # CLI command registry (Phase 4)
│   │
│   ├── handlers/                # HTTP route handlers
│   │   ├── pages/               # Page handlers
│   │   │   └── home.ts          # GET /api/home (Phase 2)
│   │   ├── posts/               # Blog post handlers (reference)
│   │   │   ├── list.ts          # GET /api/posts
│   │   │   ├── show.ts          # GET /api/posts/:slug
│   │   │   ├── create.ts        # POST /api/posts
│   │   │   ├── update.ts        # PUT /api/posts/:id
│   │   │   └── destroy.ts       # DELETE /api/posts/:id
│   │   ├── authors/             # Author handlers (reference)
│   │   │   ├── list.ts
│   │   │   ├── show.ts
│   │   │   ├── create.ts
│   │   │   ├── update.ts
│   │   │   └── destroy.ts
│   │   ├── categories/          # Category handlers (reference)
│   │   │   ├── list.ts
│   │   │   ├── show.ts
│   │   │   └── posts.ts         # GET /api/categories/:slug/posts
│   │   ├── media/               # Media/asset handlers (reference)
│   │   │   ├── upload.ts
│   │   │   ├── show.ts
│   │   │   └── destroy.ts
│   │   └── schemas/             # Schema management (reference)
│   │       ├── list.ts
│   │       ├── show.ts
│   │       ├── create.ts
│   │       ├── update.ts
│   │       └── destroy.ts
│   │
│   # commands/                  # CLI command handlers (Phase 4)
│   #   ├── posts/               # Post CLI commands
│   #   │   ├── list.ts          # cms posts:list
│   #   │   └── create.ts        # cms posts:create
│   #   ├── authors/             # Author CLI commands
│   #   │   ├── list.ts          # cms authors:list
│   #   │   └── create.ts        # cms authors:create
│   #   └── events/              # Event management commands
│   #       ├── list.ts          # cms events:list
│   #       └── replay.ts        # cms events:replay
│   │
│   ├── middleware/              # Custom middleware (HTTP)
│   │   ├── cors.ts             # CORS configuration
│   │   ├── auth.ts             # Authentication (future)
│   │   └── logging.ts          # Request logging
│   │
│   └── ui/                      # View layer (framework-agnostic)
│       ├── components/          # UI components
│       │   └── Homepage.tsx     # Homepage component (Phase 3)
│       │   # ApiProvider.tsx    # API context (reference pattern)
│       │   # ...                # Additional components (user implementation)
│       └── main.tsx             # Application entry (React/Vue)
│
...
│       └── index.ts             # Exports
│
├── package.json                  # Top-level dependencies (server + UI)
├── tsconfig.json                 # TypeScript configuration
└── vite.config.ts                # Vite configuration for UI dev server
```
...
export function registerRoutes(router: Router) {
  ...
  router.get('/api/home', import('./handlers/pages/home'));
}
```
...
export function registerRoutes(router: Router) {
  ...
  router.get('/api/posts/:slug', import('./handlers/posts/show'));
  router.post('/api/posts', import('./handlers/posts/create'));
  router.put('/api/posts/:id', import('./handlers/posts/update'));
  router.delete('/api/posts/:id', import('./handlers/posts/destroy'));

  // Author routes (for reference)
  router.get('/api/authors', import('./handlers/authors/list'));
  router.get('/api/authors/:slug', import('./handlers/authors/show'));
  router.post('/api/authors', import('./handlers/authors/create'));
  router.put('/api/authors/:id', import('./handlers/authors/update'));
  router.delete('/api/authors/:id', import('./handlers/authors/destroy'));

  // Category routes (for reference)
  router.get('/api/categories', import('./handlers/categories/list'));
  router.get('/api/categories/:slug', import('./handlers/categories/show'));
  router.get('/api/categories/:slug/posts', import('./handlers/categories/posts'));

  // Asset/media routes (for reference)
  router.post('/api/media', import('./handlers/media/upload'));
  router.get('/api/media/:id', import('./handlers/media/show'));
  router.delete('/api/media/:id', import('./handlers/media/destroy'));

  // Schema management routes (for CMS admin - reference)
...
// Multiple params
/api/categories/:categorySlug/posts/:postSlug

// Nested resources
/api/authors/:authorId/posts

// Query strings (handled via context.query)
/api/posts?page=1&limit=20&category=tech
```
...
describe('listPosts handler', () => {
  ...
  it('filters posts by query params', async () => {
    const request = new Request('http://localhost:3000/api/posts?category=tech');
    const context = { 
      params: {}, 
...
describe('API Routes', () => {
  ...
  it('GET /api/posts/:slug returns post', async () => {
    const request = new Request('http://localhost:3000/api/posts/my-first-post');
    const response = await router.handle(request);
    expect(response.status).toBe(200);
...
- [ ] Implement homepage handler (`handlers/pages/home.ts`)
- [ ] Add CORS middleware
- [ ] Add logging middleware
- [ ] Test with curl/browser

**Note**: Phase 2 focuses on minimal routing setup. Additional routes (posts, authors, etc.) shown in examples are for reference - users will implement as needed.
