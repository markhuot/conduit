...
┌──────────────────────────────────────────────────┐
│             Service Container                    │
│  (src/container.ts) ← User can modify            │
├──────────────────────────────────────────────────┤
│  • session    → { store: SessionStore }          │
│  • admin      → { layout: ComponentType }        │
│  • logging    → { verbosity: number }            │
│  • search     → SearchConfig (future)            │
│  • seo        → SEOConfig (future)               │
└──────────────────────────────────────────────────┘
                    ▲
                    │
         ┌──────────┴─────────┐
...
└────────┬────────┘
         ...
         ▼
┌────────────────────┐      No       ┌──────────────────┐
│  Auth middleware   │─────────────▶ │  Custom redirect │
│  checks session    │               │  or /admin/login │
└────────┬───────────┘               └──────────────────┘
         │ Yes
         ▼
┌────────────────────┐
...
3. Valid session → proceed to handler
4. Invalid/missing session → execute configured response (default: redirect to `/admin/login`)
...
**Benefits:**
- ✅ Cleaner API: `resolve('session')` instead of `resolve('session.config')`
- ✅ More intuitive naming
- ✅ Consistent with future additions
...
┌──────────────────────────────────────────┐
│  src/server.ts                           │
│  1. Parse CLI flags (-v, -vv, -vvv)      │
│  2. Set 'logging' in container           │
└──────────────────────────────────────────┘
                    │
                    ▼
┌──────────────────────────────────────────┐
│  logger() middleware                     │
│  1. Reads 'logging' from container       │
│  2. Returns appropriate middleware        │
└──────────────────────────────────────────┘
```
...
│  1. Registers 'admin' in container       │
│  2. Registers all admin routes           │
└──────────────────────────────────────────┘
                    │
                    ▼
┌──────────────────────────────────────────┐
│  Admin Handlers (login, dashboard, etc.) │
│  Call ui(<Component />) with no params   │
└──────────────────────────────────────────┘
                    │
                    ▼
┌──────────────────────────────────────────┐
│  ui() helper                             │
│  1. Checks for explicit layout option    │
│  2. Falls back to 'admin' from container │
│  3. Falls back to default layout         │
└──────────────────────────────────────────┘
```

**Benefits:**
- ✅ No context passing to handlers
- ✅ Layout configured once in `adminRoutes()`
- ✅ Can still override per-handler if needed
...
### 4. Session Management (lib/auth/session.ts)
...
### 5. Memory Session Store (lib/auth/stores/memory.ts)
...
### 6. Redis Session Store (lib/auth/stores/redis.ts)
...
### 7. Authentication Middleware (lib/auth/middleware.ts)
...
### 8. Admin Layout Component (lib/admin/ui/AdminLayout.tsx)
...
### 9. Admin Route Definitions (lib/admin/routes.ts)

Admin routes register layout and handlers:
 ...
 * 
 * Sets admin config in container so all admin handlers automatically use it.
 */
export function adminRoutes(group: RouteGroup): void {
  // Register admin config in container
  set('admin', {
    layout: AdminLayout,
  });
...
**Design Note**: Admin routes and handlers are part of the library, making the admin panel a drop-in feature. Layout is set in container here.

### 10. Updated ui() Helper (lib/router/react/response.tsx)
...
### 11. Login Handler (lib/admin/handlers/login.tsx)
...
### 12. Login Form Component (lib/admin/ui/LoginForm.tsx)
...
### 13. Authentication Handler (lib/admin/handlers/authenticate.ts)
...
### 14. Dashboard Handler (lib/admin/handlers/dashboard.tsx)
...
### 15. Dashboard Component (lib/admin/ui/Dashboard.tsx)
...
### 16. Logout Handler (lib/admin/handlers/logout.ts)
...
### 17. Logging Configuration (src/logging-config.ts)
...
### 18. Logging Middleware (src/middleware/logging.ts)
...
if (verbosity > 0) {
  console.log(`📊 Logging verbosity: ${verbosity}`);
}
```

**Key Points**: 
- Set logging config in container before registering routes
- Import config files at the top to register services before the router handles requests
...
│   ├── admin/                     # Admin panel (drop-in feature)
│   │   ├── routes.ts             # 🔧 Sets admin.layout in container
│   │   ├── handlers/             # Admin handlers (included in library)
│   │   │   ├── login.tsx         # Login form (GET)
│   │   │   ├── authenticate.ts   # Login processing (POST)
│   │   │   ├── dashboard.tsx     # Dashboard (GET)
│   │   │   └── logout.ts         # Logout (POST)
│   │   └── ui/                   # Admin UI components (no CSS)
│   │       ├── AdminLayout.tsx   # Admin layout component
│   │       ├── LoginForm.tsx     # Login form (no CSS)
│   │       └── Dashboard.tsx     # Dashboard (no CSS)
│   │
│   ├── auth/                      # Authentication utilities
│   │   ├── middleware.ts         # Auth middleware (requireAuth, redirectIfAuth)
│   │   ├── session.ts            # Session management (uses container)
│   │   └── stores/               # Session store implementations
│   │       ├── memory.ts         # In-memory store (dev)
│   │       └── redis.ts          # Redis store (production)
│   │
│   └── router/
│       ├── react/
│       │   └── response.tsx      # 🔧 Checks container for admin.layout
│       └── index.ts              # 🔧 Add group() method
│
├── src/                           # 🏠 USER IMPLEMENTATION
│   ├── container.ts               # 🆕 DI container (moved from lib/)
│   ├── logging-config.ts          # 🆕 Logging config interface
│   │
│   ├── handlers/
│   │   └── home.tsx              # User handlers only
│   │
│   ├── middleware/
│   │   ├── cors.ts
│   │   └── logging.ts            # 🆕 Reads logging.config from container
│   │
│   ├── routes.ts                 # 🔧 Register admin routes
│   └── server.ts                 # 🔧 Set logging.config, import configs
│
├── config/                        # 🆕 USER CONFIGURATION
│   └── session.ts                # Session storage config
│   # search.ts                   # Search provider config (future)
│   # seo.ts                      # SEO provider config (future)
```
...
- [ ] Create `lib/admin/routes.ts` that sets `admin.layout` in container
- [ ] Update `ui()` helper to check container for `admin.layout`
...
**Key Improvements**: 
- ✅ Merged authentication + layout + logging into single spec
- ✅ Container moved to `src/` for user modification
- ✅ Clean naming: `session`, `admin`, `logging` (no `.config` suffix)
- ✅ Config objects in container (not raw services)
- ✅ No CSS in initial implementation (focus on HTML structure)
- ✅ Container-based layout system (no prop drilling)
- ✅ Configurable logging with verbosity levels
- ✅ Single-line drop-in admin panel
- ✅ Clean separation of concerns
